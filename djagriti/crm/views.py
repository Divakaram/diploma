...
from crm.serializers import TeachersSerializer
...
def index(request):
    ...
    response = get_catalog("Услуги")
    dict_from_json = response.json()
    napr = dict_from_json['value']
    if request.method == 'POST':
        form = AuthForm(request.POST)
        password = request.POST['password']
        phone = request.POST['phone']
        auth = auth_user(phone, password)
        if auth:
            request.session["user_data"] = auth
...
def teachers(requst):
    form = OrderForm()
    response = get_catalog("Сотрудники", params={"$expand": "Направление"})
    dict_from_json = response.json()
    posts = dict_from_json["value"]
    return render(requst, 'crm/teachers.html', {'form': form, 'posts': posts, 'title': 'Тренеры'})


def price(request):
    form = OrderForm()
    response = get_catalog("ВидыЧленства")
    dict_from_json = response.json()
    price_site = dict_from_json['value']
...
def profile(request):
    ...
    print(user_data)
    return render(request, 'crm/profile.html', {"user_data": user_data})
...
def show_post(request, name):
    form = OrderForm()
    response = get_catalog("Сотрудники", params={"$filter": f"Description eq '{name}'", "$expand": "Направление"})
    post = response.json()["value"][0]
...
def export_to_xlsx(request):
    ...
    response['Content-Disposition'] = 'attachment; filename={date}_otchet.xlsx'.format(
